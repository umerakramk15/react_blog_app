...
function UpdatePost() {
  ...
  const [sampleFeaturedImage, setSampleFeaturedImage] = useState("");
  const [sampleEndImage, setSampleEndImage] = useState("");
  ...
  const getSinglePost = async () => {
    const { data } = await axios.get(
      `${import.meta.env.VITE_REACT_APP_API_LOCAL_HOST}/api/v1/blog/post/${
        params.slug
      }`
    );

    setTitle(data.blog.title);
    setDescription1(data.blog.description1);
    setDescription2(data.blog.description2);
    setCategory(data.blog.category.title);
    console.log("I'm Featured", featuredImage);
    setSampleFeaturedImage(data.blog.featuredImage);
    setSampleEndImage(data.blog.endImage);
    setQuote(data.blog.quote);
  };
    ...
    <div className=" w-[100%] h-[100%] overflow-hidden">
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="grid col-span-12 sm:col-span-12 w-full  gap-8">
          {/* Inputs  */}
          <h1 className="text-2xl md:text-4xl">Update Post</h1>
          <form onSubmit={handleCreatePost} className="flex flex-col gap-3">
            ...
            <div>
              <h1 className="py-4 text-xl md:text-2xl">Ending Image</h1>
              <div className="w-[200px] rounded-2xl overflow-hidden">
                {endImage ? (
                  <>
                    <div>
